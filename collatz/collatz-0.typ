...
#spoiler([Can these Collatz sequences be generalized to other numbers?], [Yes! For example, instead of multiplying by 3 and adding 1, we can see what happens when we change this rule to multiply by a number $k$ other than 3. Depending on the number you choose, behavior changes drastically. For instance, if you choose an even $k$, sequences behave trivially#footnote[They always diverge to infinity.]. For this reason, mathematicians studying generalized Collatz mappings exclusively focus on odd values of $k$. I'd like to refer you to #link("https://arxiv.org/html/2005.00346v3")[an article] that discusses this in detail.])

#spoiler([What happens if we use numbers other than integers?], [Let's say we use numbers other than integers, such as real numbers, which include $3/4$, $5.8$, and even $pi$. Then the strict definition of _odd_ and _even_ breaks: how would you define $pi$ as an even or odd number? There are ways to get around this while remaining mathematically correct (for example, by using continuous Collatz functions). The formalization of this generalization is beyond the scope of this post, but the key point to remember is that the behavior will be much more chaotic (a starting value of $2.00000$ goes to 1; but a starting value of $2.00001$ might fluctuate wildly and shoot up into millions, or fall into a completely random decimal loop).])

#spoiler([Maybe we just can't solve this problem at all?], [In 1972, the famous and brilliant mathematician John Conway proved that a natural generalization of the Collatz problem is algorithmically undecidable. That means no computer program can determine the fate of every generalized Collatz-like function, such as we have seen in the questions above. Conway's proof applies to a specific family of generalized functions, not the _original_ function itself where you multiply by 3 and add 1. However, it remains highly possible that the _original_ Collatz conjecture itself is undecidable, and hence unprovable within standard mathematics or computation theory. And, in fact, several prominent mathematicians share this view. This is a grim scenario that we must, unfortunately, take seriously. There is still the possibility of proving that this problem is undecidable, but it seems extremely difficult to construct such a proof.])
