...
#spoiler([If we could prove that $"P" = "NP"$, would that have any implications for the other six Millennium Problems?], [Yes. Stephen Cook, who introduced this problem at the Clay Institute in 1971, believed that such a proof would have both negative and positive consequences. The negative consequences would indeed affect the security of our current computer systems, which rely on cryptography based on complex problems (these problems would thus become easy to solve, and therefore no longer provide any security). The positive consequences, on the other hand, would concern mathematics: Cook already asserted in his 1971 seminal paper that if $"P" = "NP"$, then finding formal mathematical proofs (of reasonable size) in polynomial time would be possible, and this therefore applies to the other Millennium Problems. However, Grigori Perelman, a Russian mathematician, did not wait for $"P" = "NP"$ to prove one of the seven problems in 2003 and also to decline the \$1 million prize.])

#spoiler("Are there other complexity classes?", [Actually, there is an entire hierarchy of complexity classes. Just as with #smallcaps[P] and #smallcaps[NP], the relationships between these other classes are still not known. Furthermore, complexity classes and hierarchies depend on the computational model used. For #smallcaps[P] and #smallcaps[NP], the models are based on so-called deterministic machines: either an instruction in the algorithm is executed, or it is not. However, there are other computational models, such as probabilistic models. Such models are used, for example, to classify the complexity classes of quantum algorithms. Indeed, quantum computers perform calculations using probabilities; they are therefore not deterministic machines. Consequently, problems solved by such probabilistic machines are classified into classes such as #smallcaps[BPP] or, more accurately, #smallcaps[BQP] when referring to quantum machines. If you're interested, one of the major problems in quantum complexity theory is #smallcaps[BQP] vs. #smallcaps[QMA], which is the equivalent of the #smallcaps[P] vs. #smallcaps[NP] problem for quantum systems.

And if you're now fascinated by complexity theory and the "zoologization" of problems, check out the #link("https://complexityzoo.net/Complexity_Zoo")[Complexity Zoo], which lists the classes and hierarchies currently known to researchers.

#figure(image("complexity-classes.png", width: 70%, height: 70%), caption: [The most important complexity classes. Source of this great diagram is #link("https://blog.ohaithe.re/post/655534886524960768/slide-from-my-candidacy-exam-showing-the")[Oh Hai There (Blog)].])
